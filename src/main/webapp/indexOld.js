document.addEventListener("DOMContentLoaded", function(event) {
  ...
  var submit = function(){
    let  url = "localhost:8080/WebApplication1/ws/users?username="+data.username+"&password=" +data.password;
    fetch(url, {
      method: "POST",
      cache: 'no-cache'
    })
        ...
        if (Object.keys(result.userss).length ===0){
          ...
          localStorage.setItem("user", JSON.stringify(result.userss.users))
            if(JSON.parse(localStorage.user).departmentId.name==='Admin'){
              window.location.replace("localhost:8080/WebApplication1/MainPage.html")
            } else {
